...
*/
  ...
  d3.select("#data_nodes_group")
      .attr("transform", "translate(" + zoom.translate() + ")scale(" + zoom.scale() + ")");
  
  d3.selectAll(".data_node")
      .attr("r", 25/zoom.scale()); // 0.0022; 
...
d3.csv("t13mn01.csv", function(error, data) {
...
*/
	    ...
	    var path_tolerance = 30; //user changeable (add slider or something) 
...
/* 	            	console.log('selected node with dist2path of ', d.dist2path); */
	    ...
	    function update_scatter_plot() {
	        ...
	        /* 	console.log('branch ' ,  branch.order, 'level',  branch.level,  'delta', branch.delta , 'source', branch.source_order) */
...
/* 					.domain([-1.6,-1.8 ]) */
           ...
           yaxes
			...
			var to_plot = user_data.filter(function(d) {
	                return d.selected
	            });
